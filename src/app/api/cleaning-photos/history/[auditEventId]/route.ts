...
import { getHistoryPhotoRef } from "@/lib/services/cleaningService";

const PHOTO_DIR = path.join(process.cwd(), "data", "private-uploads", "cleaning-photos");

export async function GET(req: NextRequest, ctx: RouteContext<"/api/cleaning-photos/history/[auditEventId]">) {
  const user = await getCurrentUser();
  if (!user) return new Response("Unauthorized", { status: 401 });

  const { auditEventId } = await ctx.params;
  const { searchParams } = new URL(req.url);
  const kind = searchParams.get("kind") === "before" ? "before" : "after";
  const download = searchParams.get("download") === "1";

  const photo = getHistoryPhotoRef(auditEventId, user.storeId, kind);
