...
export default function CompletedTaskRow({ task, lang, storeId, from }: { task: CompletedTaskData; lang: Language; storeId: string; from?: string }) {
  ...
  return (
    <Link href={from ? withFrom(`/task/${task.id}`, from) : `/task/${task.id}`} className="flex items-center gap-2.5 px-3 py-2">
