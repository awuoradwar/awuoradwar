...
export async function GET(_req: NextRequest, ctx: RouteContext<"/api/cleaning-photos/[taskId]">) {
  const user = await getCurrentUser();
  if (!user) return new Response("Unauthorized", { status: 401 });

  const { taskId } = await ctx.params;
  const photo = getPhotoRefForTask(taskId, user.storeId);
  ...
  try {
    ...
    return new Response(new Uint8Array(data), {
      headers: {
        ...
        "Content-Disposition": `inline; filename="${photo.photo_url}"`,
